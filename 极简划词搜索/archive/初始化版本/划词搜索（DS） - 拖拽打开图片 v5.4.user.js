// ==UserScript==
// @name         极简划词工具栏（打开+复制+百度+知乎+头条+小红书）· 拖拽搜索版（支持链接/图片拖拽打开）
// @description  超小尺寸悬浮窗，支持划词搜索、复制、打开链接；拖拽链接/图片直接新窗口打开，拖拽选中文本则执行搜索或打开域名
...
// @note         2025.2.22 V5.3 增加链接拖拽打开功能
// @note         2025.2.23 V5.4 增加图片拖拽打开功能
// @note         2025.2.24 V5.5 增加小红书搜索引擎
// ==/UserScript==
...
(function() {
    ...
    function isLinkElement(el) {
        ...
        while (parent) {
            if (parent.tagName === 'A' && parent.href) return true;
    ...
    function populateToolbar(selectedText) {
        ...
        buttons.forEach((btn, index) => {
            if (index > 0) toolbar.appendChild(createSeparator());
    ...
    let dragUrl = null;        // 如果是链接或图片，保存URL
