// ==UserScript==
// @name         极简划词工具栏（打开+复制+百度+知乎+头条）· 拖拽搜索版
// @description  超小尺寸悬浮窗，支持划词搜索（百度/知乎/头条）、复制、打开链接，并增加拖拽搜索功能（选中后左键拖拽触发百度搜索或打开域名）
// @icon         https://www.baidu.com/favicon.ico
// @namespace    http://tampermonkey.net/
// @version      5.2
// @description  超小尺寸，悬浮窗 + 拖拽搜索，修复点击复制按钮触发拖拽的bug
// @author       ddrwin
// @match        *://*/*
// @grant        none
// @note         2025.2.18 V4.0 初始版本，支持复制和百度搜索
// @note         2025.2.19 V4.4 增加打开按钮，优化样式，添加知乎、头条
// @note         2025.2.20 V5.0 增加拖拽搜索功能，修复状态残留bug
// @note         2025.2.21 V5.2 修复点击复制按钮误触拖拽搜索的问题
// ==/UserScript==
...
(function() {
    ...
    function createButton(iconDataUrl, text, clickHandler) {
        ...
        btn.addEventListener('mouseenter', (e) => {
            ...
            e.currentTarget.style.color = '#5FE382';      // 文字颜色调整为更亮绿色
            img.style.filter = GREEN_FILTER;
        });
    ...
    let dragListening = false; // 是否正在监听拖拽
    let dragTriggered = false; // 是否已触发拖拽，用于防止重复

    function startDragListening(e) {
        // 只有左键按下
        if (e.button !== 0) return;

        // 忽略在工具栏内的鼠标按下（防止点击按钮时误触发）
        if (toolbar && toolbar.contains(e.target)) return;

        // 检查是否已有选中文本且非空
        const selection = window.getSelection();
        const selectedText = selection.toString().trim();
        if (!selectedText || isInsideEditable(e.target)) return;
    ...
    function onDragMove(e) {
        ...
        if (distance >= DRAG_THRESHOLD) {
            ...
            // 延迟重置 dragTriggered，确保后续事件不受影响（即使 mouseup 未触发）
            setTimeout(() => {
                dragTriggered = false;
            }, 100);
        }
    ...
    // ==================== 事件监听 ====================
    // 监听 mousedown 启动拖拽检测
    document.addEventListener('mousedown', startDragListening);

    // 原有的 mouseup 用于显示悬浮窗（需确保不被拖拽触发干扰）
    document.addEventListener('mouseup', (e) => {
        // 如果拖拽已触发，则不再显示悬浮窗，并重置标志
        if (dragTriggered) {
            dragTriggered = false;
            return;
        }
