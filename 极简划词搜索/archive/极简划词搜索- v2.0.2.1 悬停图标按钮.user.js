// ==UserScript==
// @name         极简划词搜索
// @description  超小尺寸悬浮窗，支持划词搜索（百度/头条/知乎/小红书）、复制、打开链接；拖拽链接/图片直接新窗口打开，拖拽选中文本则执行搜索或打开域名；图片悬停显示保存按钮（支持选择保存位置，取消不会误下载）；极致性能优化，大幅降低CPU占用
...
// @note         2026.2.28 V2.1.0 新增图片悬停保存按钮，样式与现有按钮一致；点击保存优先弹出系统保存位置对话框，兼容传统下载
// @note         2026.3.1  V2.1.1 修复取消保存对话框后误触发下载的问题；优化图片悬停监听为 mouseenter/leave，减少卡顿
...
(function() {
    ...
    // 改进的下载函数：优先使用 showSaveFilePicker 弹出保存对话框，否则回退到 <a download>
    async function downloadImage(url) {
        if (window.showSaveFilePicker) {
            try {
                ...
                console.log('showSaveFilePicker failed, fallback to <a download>', err);
            }
        }
    ...
    function onImageMouseEnter(e) {
        ...
        if (isInsideEditable(img)) return;
        ...
        imageHoverTimer = setTimeout(() => {
            ...
            hideToolbar();
            showToolbar(e.clientX, e.clientY, src, 'image');
    ...
    // 图片悬停监听（使用 mouseenter/mouseleave 减少冒泡）
    document.addEventListener('mouseenter', onImageMouseEnter, { passive: true });
    document.addEventListener('mouseleave', onImageMouseLeave, { passive: true });
