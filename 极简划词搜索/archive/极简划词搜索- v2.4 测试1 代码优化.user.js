...
(function() {
     ...
     * @type {number}
     */
    const OFFSET_LEFT_ICON_COUNT = 0;
     ...
     * @type {number}
     */
    const OFFSET_DOWN_ICON_COUNT = 0;
    ...
    /**
     * 保存用户配置的搜索引擎名称列表
     * @param {string[]} engineNames - 要保存的搜索引擎名称数组
     */
    ...
    /**
     * 当前启用的搜索引擎名称列表（从 localStorage 加载）
     * @type {string[]}
     */
    ...
    // ==================== 按钮顺序配置 ====================
    /**
     * 从 localStorage 加载按钮顺序配置
     * @returns {'copy-first' | 'highlight-first'} 按钮顺序
     */
    function loadButtonOrderConfig() {
        const saved = localStorage.getItem('button_order_config');
        if (saved === 'highlight-first') return 'highlight-first';
        return 'copy-first'; // 默认复制在前
    }

    /**
     * 保存按钮顺序配置
     * @param {'copy-first' | 'highlight-first'} order - 按钮顺序
     */
    function saveButtonOrderConfig(order) {
        localStorage.setItem('button_order_config', order);
    }

    /**
     * 当前按钮顺序配置
     * @type {'copy-first' | 'highlight-first'}
     */
    let buttonOrder = loadButtonOrderConfig();

    /**
     * 获取应在工具栏显示的搜索引擎列表（完全基于用户配置，无域名特殊处理）
     * @returns {Array<{name: string, icon: string, url: string}>}
     */
    function getEnabledEnginesForCurrentSite() {
        return ALL_SEARCH_ENGINES.filter(engine => enabledEngineNames.includes(engine.name));
    }

    // ==================== 高亮管理 ====================
    /**
     * 存储当前所有需要高亮的关键词及其对应的样式索引
     * @type {Array<{keyword: string, styleIndex: number}>}
     */
    let highlights = [];
    ...
    // ==================== 设置面板 ====================
    /** @type {HTMLDivElement|null} 设置面板 DOM 元素 */
    ...
    function createSettingsPanel() {
        ...
        orderCheckbox.checked = (buttonOrder === 'highlight-first');
        orderCheckbox.style.marginRight = '6px';
        ...
        settingsPanel.appendChild(orderDiv);
        ...
        saveBtn.addEventListener('click', () => {
            ...
            // 保存按钮顺序配置
            const orderCheckbox = checkboxes[ALL_SEARCH_ENGINES.length];
            buttonOrder = orderCheckbox.checked ? 'highlight-first' : 'copy-first';
