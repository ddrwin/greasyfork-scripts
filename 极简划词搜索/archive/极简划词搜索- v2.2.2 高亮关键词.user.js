...
(function() {
    ...
    const TOOLBAR_HIDE_MODE = 'manual'; // 默认手动隐藏（需点击页面其他位置或按钮后才隐藏）
    const HIDE_DELAY = 1000; // 自动隐藏延迟（毫秒），仅在 TOOLBAR_HIDE_MODE='auto' 时有效

    // 是否在页面加载时自动高亮预定义关键词（默认关闭）
    const AUTO_HIGHLIGHT_ENABLED = false;

    // ==================== 高亮分组（样式已按新顺序调整，关键词已清空，可自行添加）====================
    // 注：字体加粗样式默认被注释，如需启用请去掉 /* 和 */
    const HIGHLIGHT_GROUPS = [
        {
            name: "红字黄底",
            keywords: [], // 请在此添加关键词，例如 ["北京", "阿里"]
    ...
    let observerDebounceTimer = null; // MutationObserver 防抖定时器
    ...
    function addDynamicHighlight(keyword) {
        if (!keyword) return;
        // 去重：如果已存在相同关键词，则将其移动到末尾（保留原样式，仅改变顺序）
        const existingIndex = highlights.findIndex(h => h.keyword === keyword);
        if (existingIndex !== -1) {
            // 已存在，则将其移动到末尾（表示最近使用）
    ...
    // 当页面动态添加内容时，自动对新内容进行高亮（增量模式），并加入防抖避免频繁触发
    function initMutationObserver() {
        observer = new MutationObserver(mutations => {
            // 防抖：延迟 200ms 后执行，避免短时间内大量触发
