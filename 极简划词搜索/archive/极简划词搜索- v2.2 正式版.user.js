...
// @note         2026.3.1  V2.1.1 增强图片下载：增加 CORS 代理重试机制、更完善的请求头，提升跨域图片保存成功率
// @note         2026.3.1  V2.2 工具模块拆分（UrlUtils）、使用可选链和空值合并简化代码、代理请求封装为独立函数
...
(function() {
    ...
    // ==================== 可配置常量 ====================
    /**
     * 当直接 fetch 图片失败时，是否尝试使用 CORS 代理。
     * 设置为有效的代理 URL 即可启用，置空字符串 '' 可禁用代理功能。
     * 注意：公共代理可能不稳定，如有隐私顾虑请自建代理或置空。
     */
    const PROXY_URL = 'https://cors-anywhere.herokuapp.com/';

    /**
     * 是否在最后的手动保存提示中自动打开新窗口。
     * 设为 true 则会在控制台提示的同时打开图片新窗口（便于手动保存）；
     * 设为 false 则仅给出控制台提示。
     */
     ...
     * 每项包含：
     *   name: 显示在按钮上的名称
     *   icon: 图标（通常使用 ICONS.search 即可，也可自定义）
     *   url:  搜索链接，其中 %s 会被替换为编码后的搜索词
     * 例如添加必应：{ name: '必应', icon: ICONS.search, url: 'https://www.bing.com/search?q=%s' }
     */
    ...
    /**
     * 异步下载图片，增强版：
     * 1. 先尝试直接 fetch（带完整请求头）
     * 2. 若失败且配置了 PROXY_URL，则通过代理重试
     * 3. 最终回退到 <a download> 和手动提示
     * @param {string} url - 图片URL
     */
