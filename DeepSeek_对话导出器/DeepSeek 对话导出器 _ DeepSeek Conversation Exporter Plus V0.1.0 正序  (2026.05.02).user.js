...
(function() {
    ...
    let state = {
        ...
        reverseOrder: localStorage.getItem('ds_export_reverse_order') === 'true'   // 新增：导出顺序反转
    };
    ...
    function convertToMd(d) {
        try {
            ...
            // 根据开关决定是否反转消息顺序
            const msgs = state.reverseOrder ? [...d.chat_messages].reverse() : d.chat_messages;
    ...
    window.addEventListener('load', () => {
        ...
        // 新增：导出顺序反转菜单
        GM_registerMenuCommand(state.reverseOrder ? '✅ Reverse Order (Oldest First)' : '❌ Normal Order (Newest First)', () => {
            state.reverseOrder = !state.reverseOrder;
            localStorage.setItem('ds_export_reverse_order', state.reverseOrder);
            location.reload();
        });
    });
})();