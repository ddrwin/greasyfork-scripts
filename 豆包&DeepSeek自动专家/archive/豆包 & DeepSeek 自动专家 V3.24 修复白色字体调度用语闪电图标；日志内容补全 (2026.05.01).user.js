// ==UserScript==
// @name         豆包 & DeepSeek 自动专家 V3.24
// @namespace    https://github.com/ddrwin/ai-auto-expert
// @version      3.24
// @description  统一偏好管理、模块化站点控制、遥测拦截。V3.24：修复白色字体问题；调度→调度中心；闪电图标用于成功动作；日志文字补全。
...
// @license      MIT
// @note         2026.05.01 V3.24 修复白色字体/调度用语/闪电图标；日志内容补全。
// ==/UserScript==
...
(function() {
    ...
    const LOG = {
        ...
        scheduler(msg) {
            console.log(`${ts()} %c⚙️ [调度中心] %c${msg}`, 'color:#fff;background:#2c3e50;padding:1px 4px;border-radius:2px', 'color:#ccc');
        },
        step(label, msg) {
        ...
        ok(label, msg) {
            const c = getColor(label.split(' ')[0]);
            // 修复白色字体问题，文字颜色改为黑色
            console.log(`${ts()} %c✅ [${label}] %c${msg}`, `color:#fff;background:${c.main};padding:1px 4px;border-radius:2px`, 'color:#1a1a1a');
        },
        fail(label, msg) {
            const c = getColor(label.split(' ')[0]);
            // 修复白色字体问题，文字颜色改为黑色
            console.log(`${ts()} %c❌ [${label}] %c${msg}`, `color:#fff;background:${c.dark};padding:1px 4px;border-radius:2px`, 'color:#1a1a1a');
        },
        manual(msg) {
            console.log(`${ts()} %c👆 [手动] %c${msg}`, 'color:#fff;background:#722ed1;padding:1px 4px;border-radius:2px', 'color:#000');
        },
        divider() {
        ...
        async function applyControl(control) {
            ...
            if (control.id === 'modelSelect' || control.id === 'doubaoModel') {
                ...
                if (currentModel === target) {
                    ...
                    LOG.action(label, `执行点击操作，从 ${currentModel} 切换到 ${target}`);
                ...
                if (ariaOk && classOk && indexOk) {
                    // 闪电图标用于成功动作
                    LOG.action(label, `切换成功，模型已生效：${target}，金标准三重验证通过`);
                } else {
            ...
            for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
                ...
                if (el && control.isMatch(el, target)) {
                    LOG.ok(label, `已处于目标状态，无需操作 (${target})`);
                    return;
                }
                if (el) break;
            ...
            LOG.action(label, `执行切换操作，目标状态：${target}`);
            await control.action(target, el);
            // 闪电图标用于成功动作
            LOG.action(label, `切换成功，当前状态为：${target}`);
        }
