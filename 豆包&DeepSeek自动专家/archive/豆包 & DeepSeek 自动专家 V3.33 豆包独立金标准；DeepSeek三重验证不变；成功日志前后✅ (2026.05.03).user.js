// ==UserScript==
// @name         豆包 & DeepSeek 自动专家 V3.33
// @namespace    https://github.com/ddrwin/ai-auto-expert
// @version      3.33
// @description  统一偏好管理、模块化站点控制、遥测拦截。V3.33：豆包独立金标准(data-checked+文字)；DeepSeek保持三重验证；成功日志前后✅。
...
// @license      MIT
// @note         2026.05.03 V3.33 豆包独立金标准；DeepSeek三重验证不变；成功日志前后✅。
// ==/UserScript==
...
(function() {
    ...
    function createSiteController(config) {
        ...
        function identifyPage() {
            ...
            if (hostname.includes('doubao.com')) {
                if (location.pathname === '/chat' || location.pathname === '/chat/') return 'home';
                return 'unknown';
            }
        ...
        async function applyControl(control) {
            ...
            // ----- 豆包模型 5 步流程（独立金标准：文字 + data-checked）-----
            if (control.id === 'doubaoModel') {
                ...
                for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
                    btn = control.findElement();
                    if (btn) {
                        currentModel = control.getCurrentValue(btn);
                        if (currentModel) {
                ...
                if (currentModel === target) {
                    ...
                    await control.action(target, btn);
                    LOG.wait(label, '点击已完成，等待 300ms 后进行金标准验证...');
                    await new Promise(r => setTimeout(r, 300));
                }

                // 豆包金标准验证：文字 + data-checked
                LOG.step(`${label} 5/${totalSteps}`, '金标准验证（文字 + data-checked）');
                const verifyBtn = control.findElement();
                if (verifyBtn) {
                    const verifyText = control.getCurrentValue(verifyBtn);
                    const dataChecked = verifyBtn.getAttribute('data-checked');
                    LOG.scan(label, `按钮文字匹配(${verifyText}/${target}): ${verifyText === target ? '✅' : '❌'}`);
                    LOG.scan(label, `data-checked属性: ${dataChecked === 'true' ? '✅' : '❌'}`);
                    if (verifyText === target) {
                        LOG.ok(label, `切换成功，模型已生效：${target}，金标准验证通过`);
                    } else {
                        LOG.fail(label, '金标准验证失败，等待下次触发重试');
                    }
                } else {
                    LOG.fail(label, '金标准验证失败：切换后无法找到按钮元素');
                }
                return;
            }

            // ----- DeepSeek 模型切换 5 步流程（三重金标准）-----
