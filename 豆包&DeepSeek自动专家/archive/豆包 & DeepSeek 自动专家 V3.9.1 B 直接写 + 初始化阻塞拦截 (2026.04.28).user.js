...
// @version      3.9.1-B
// @description  统一偏好管理、模块化站点控制、遥测拦截。豆包默认思考，DeepSeek 默认专家+深度思考。模型切换采用直接写策略。拦截页面初始化阻塞请求。
...
// @license      MIT
// @note         2026.04.29  V3.9.1-B  新增拦截 hif-dliq.deepseek.com，消除其 DNS 解析失败造成的页面初始化阻塞，提升脚本生效速度。模型切换采用直接写，不依赖异步属性。
// ==/UserScript==
...
(function() {
    ...
    const siteConfigs = {
        ...
        'chat.deepseek.com': {
            ...
            `,
            blockedHosts: ['hif-dliq.deepseek.com'],  // 拦截页面初始化阻塞请求，加速渲染
            controls: [
                {
                    id: 'modelSelect',
