...
// @note         2026.04.28  V3.7  修复 DeepSeek 模型切换在页面点击不记忆的 bug（改用异步读取最终状态）；增加鼠标悬停/移动时的自动同步抑制，防止 hover 效果干扰脚本误判；相关代码增加详细注释
// ==/UserScript==
...
(function() {
    ...
    function createSiteController(config) {
        ...
        async function applyControl(control) {
            // 如果用户正在移动鼠标，很可能只是划过某个模型选项（hover 效果），此时不触发自动同步，
            // 避免将临时视觉状态误认为用户意图。只有鼠标静止一段时间后，才认为页面稳定。
            const MOUSE_IDLE_THRESHOLD = 300; // 毫秒
            if (Date.now() - lastMouseMove < MOUSE_IDLE_THRESHOLD) {
                return; // 鼠标活跃，跳过自动同步，防止 hover 干扰
            }
        ...
        document.addEventListener('click', function(e) {
            ...
            // 【关键修复】DeepSeek 模型切换的异步处理
            // 原因：模型选择器 DOM 层级较深，点击可能落在内部元素；且页面 hover 动效可能临时改变状态。
            // 因此不依赖 closest('div[data-model-type]') 同步读取，而是延迟读取 DOM 最终有效状态。
            if (hostname.includes('deepseek.com')) {
                const modelArea = e.target.closest('div[data-model-type="expert"], div[data-model-type="default"]');
                if (modelArea) {
                    // 延迟一个微任务，等待 DeepSeek 页面自身的状态更新完毕（通常为 aria-checked 切换）
                    setTimeout(() => {
                        ...
                        if (finalValue && finalValue !== prefs.deepseekModel) {
                            ...
                            console.log(`👆 手动更新偏好: deepseekModel = ${finalValue} (异步)`);
                        }
                    }, 0);
                }
            }
