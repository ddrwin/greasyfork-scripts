// ==UserScript==
// @name         豆包 & DeepSeek 自动专家 (B-直接写版)
// @namespace    https://github.com/ddrwin/ai-auto-expert
// @version      3.9-B
// @description  统一偏好管理、模块化站点控制、遥测拦截。豆包默认思考，DeepSeek 默认专家+深度思考。模型切换采用直接写策略，不依赖异步属性。
...
// @license      MIT
// @note         2026.04.28  V3.9-B  模型切换采用直接写策略：跳过 getCurrentValue 读取，直接按存储偏好点击目标按钮。避免因 aria-checked 异步设置导致的自动恢复失败。人机对抗由冷却期 + manualTrigger 即时更新偏好保障。
// ==/UserScript==
    ...
    function createSiteController(config) {
        ...
        async function applyControl(control) {
            ...
            if (control.id === 'modelSelect') {
                ...
                if (!targetEl) return; // 目标元素还没渲染出来，等下次 scan
                // 如果已经是选中状态就不重复点（减少不必要的 DOM 操作）
                if (targetEl.getAttribute('aria-checked') === 'true') return;
                targetEl.click();
                console.log(`✅ 已应用偏好: ${control.id} = ${target} (直接写)`);
                return;
            }
