// ==UserScript==
// @name         豆包 & DeepSeek 自动专家 V3.25
// @namespace    https://github.com/ddrwin/ai-auto-expert
// @version      3.25
// @description  统一偏好管理、模块化站点控制、遥测拦截。V3.25：统一图标(步骤=⚡,成功=✅)；修复白色字体；明确深度思考/智能搜索最终结构。
...
// @license      MIT
// @note         2026.05.01 V3.25 图标统一+白色字体彻底修复+深度思考/智能搜索最终步骤明确。
// ==/UserScript==
        ...
        async function applyControl(control) {
            ...
            for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
                ...
                if (el && control.isMatch(el, target)) {
                    // 最终结论：已处于目标状态
                    LOG.ok(label, `最终确认：已处于目标状态，无需操作 (${target})`);
                    return;
                }
                if (el) break;
            ...
            if (!el) {
                LOG.fail(label, '最终确认：无法找到控件，等待下次触发');
                return;
            }

            LOG.step(`${label} ${totalSteps}/${totalSteps}`, `执行切换操作，目标状态：${target}`);
            await control.action(target, el);
            // 最终结论：切换成功
            LOG.ok(label, `最终确认：切换成功，当前状态为：${target}`);
        }
