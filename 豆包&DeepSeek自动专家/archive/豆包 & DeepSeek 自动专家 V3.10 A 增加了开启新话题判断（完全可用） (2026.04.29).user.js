// ==UserScript==
// @name         豆包 & DeepSeek 自动专家 (A-增强重试版 + 智能对话页识别)
// @namespace    https://github.com/ddrwin/ai-auto-expert
// @version      3.10-A
// @description  统一偏好管理、模块化站点控制、遥测拦截。豆包默认思考，DeepSeek 默认专家+深度思考。模型切换仅在首页生效，对话页自动跳过；全控件增强重试；拦截页面初始化阻塞请求。
...
// @license      MIT
// @note         2026.04.29  V3.10-A  新增对话页判断（/a/chat/s/）自动跳过模型切换；遥测拦截增加去重哨兵；手动捕获增加防错处理。
// ==/UserScript==
    ...
    function createSiteController(config) {
        ...
        async function applyControl(control) {
            ...
            if (control.id === 'modelSelect') {
                if (isConversationPage) return; // 对话页无模型选择器，直接退出
