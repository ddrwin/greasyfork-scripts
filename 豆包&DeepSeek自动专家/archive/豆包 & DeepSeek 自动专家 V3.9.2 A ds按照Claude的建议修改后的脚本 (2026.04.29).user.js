// ==UserScript==
// @name         豆包 & DeepSeek 自动专家 (A-增强重试版 + 全控件重试)
// @namespace    https://github.com/ddrwin/ai-auto-expert
// @version      3.9.2-A
// @description  统一偏好管理、模块化站点控制、遥测拦截。豆包默认思考，DeepSeek 默认专家+深度思考。模型切换+深度思考+智能搜索全部采用增强重试机制。拦截页面初始化阻塞请求。
...
// @license      MIT
// @note         2026.04.29  V3.9.2-A  深度思考、智能搜索也加入增强重试，消除与模型切换的速度差，提升切换体感一致性。
// ==/UserScript==
...
(function() {
    ...
    function createSiteController(config) {
        ...
        async function applyControl(control) {
            ...
            // ===== 深度思考 / 智能搜索 / 豆包模型：元素查找 + 状态匹配重试 =====
            // 之前只依赖 MutationObserver 被动触发，元素未就绪就放弃。
            // 现在加入主动重试，与模型切换保持一致的响应速度。
            let el = null;
            for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
                el = control.findElement();
                if (el && control.isMatch(el, prefs[control.storageKey])) {
                    // 元素已就绪且状态已匹配，不需要操作
                    return;
                }
                if (el) break; // 元素就绪但状态不匹配，跳出循环去执行切换
                // 元素还没渲染出来，等 200ms 再试
                await new Promise(r => setTimeout(r, RETRY_DELAY));
            }
