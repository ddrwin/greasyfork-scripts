...
(function() {
    ...
    const defaultPrefs = Object.freeze({
        ...
        deepseekModel: 'expert',          // DeepSeek：'expert' | 'fast'
    ...
    function loadPrefs() {
        ...
        if (typeof saved.deepseekExpert === 'boolean' && !saved.deepseekModel) {
            saved.deepseekModel = saved.deepseekExpert ? 'expert' : 'fast';
            ...
            controls: [
                {
                    ...
                    // 查找所有模型单选按钮（专家、快速）
                    findElement: () => document.querySelector('div[data-model-type="expert"], div[data-model-type="fast"]'),
                    getCurrentValue: () => {
                        ...
                        const fast = document.querySelector('div[data-model-type="fast"]');
                        if (fast && fast.getAttribute('aria-checked') === 'true') return 'fast';
                        // 如果都没找到，可能页面未加载完成
                        return null;
                    },
                    isMatch: (_, pref) => {
                        const cur = this.getCurrentValue(); // 注意作用域，稍后在 applyControl 中修正
                        return cur === pref;
                    },
                    action: async (pref) => {
                    ...
                    manualTrigger: (e) => {
                        ...
                        if (type === 'expert' || type === 'fast') return type;
                        return null;
                    }
                },
                {
