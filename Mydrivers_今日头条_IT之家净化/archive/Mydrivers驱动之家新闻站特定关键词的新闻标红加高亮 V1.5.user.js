...
(function() {
    ...
    //首页屏蔽关键词，把相关新闻屏蔽掉
    var BanText = ["小米", "红米", "雷军", "Redmi", "畅享", "OPPO", "vivo", "卢伟冰", "紫米", "MIUI", "一加","realme", "长城","腾讯", "魅族", "tcl", "苏宁", "华米", "iQOO", "ROG", "华米"]; //屏蔽的字符
    ...
    //首页添加关键词，高亮颜色可以自定义
    var ColorText = ["5G","华为","HMS","鸿蒙","HUAWEI","麒麟","光刻机","Mate","酷睿","HarmonyOS","荣耀","芯片","机器人","Wi-Fi"]; //高亮的字符
    var checkHigh = window.location.href.match(/mydrivers\.com\/$/);//检查首页
    if (checkHigh) {
        ColorText.map(function(i) {
    ...
    //首页添加关键词，高亮颜色可以自定义
    var ColorText = ["苹果", "iPhone", "iPad", "ios", "iOS", "微信", "Win 11","Win11","Windows 11","Windows11"]; //高亮的字符
    var checkHigh = window.location.href.match(/mydrivers\.com\/$/);//检查首页
    if (checkHigh) {
        ColorText.map(function(i) {
