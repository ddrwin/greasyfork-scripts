...
// @note         2026.02.21 V2.0 核心更新：新增强制刷新机制（点击翻页/更多/搜索触发），优化异步刷新后按钮稳定性，完善快科技搜索中转，修复层级遮挡问题，整合所有功能优化执行效率；修复首页跳转脚本未运行问题，匹配规则统一为 *://www.baidu.com/*；整合自动翻页功能，优化Google翻页按钮显示及内容拼接逻辑，模块化重构代码结构；优化Google搜索框插入位置（靠右），翻页选择器增强。
// @note         2026.02.24 V2.0.2 修复 Google 古老页面翻页问题：使用精准 XPath 定位 footer 内的下一页链接和 main 内的搜索结果项。
// @note         2026.02.24 V2.0.6 采纳用户提供的精准 XPath 规则，彻底解决古老 Google 页面翻页问题。
// @note         2026.02.24 V2.1.0 合并新旧版 Google 翻页逻辑：通过多候选选择器自动适配新旧页面；统一域名匹配规则（支持所有 Google/Bing 国家域名），添加排除非搜索页面规则。
// @note         2026.02.24 V2.1.1 增强新版 Google 翻页选择器，增加对英文 aria-label="Next page" 的支持，并添加更多候选链接，提升兼容性。
// @note         2026.02.24 V2.1.2 修复因复制脚本导致的语法错误，移除 afertPagerAutoCallFunc 中的脚本复制逻辑，避免页面报错。
// @note         2026.02.24 V2.1.3 增强 Bing 翻页适配：合并多候选选择器（支持新旧版 Bing 页面），实现双保险。
// ==/UserScript==
...
(function() {
    ...
    class SitePagerConfig {
        ...
        _getFullPagerConfig() {
            ...
            const configs = {
                ...
                google: {
                    // 翻页链接候选：优先尝试新版选择器，增加英文版
                    nextLink: [
                        "css;a[aria-label='下一页'], a[aria-label='Next page'], a#pnnext",
                        "css;a.nBDE1b.G5eFlf[aria-label='下一页']", // 旧版样式
                        "id('main')/footer//a[@aria-label and contains(@href, 'start=') and contains(., '>')]" // 旧版精确XPath
                    ],
                    // 页面元素候选：先新版，再旧版
                    pageElement: [
                    ...
                    afertPagerAutoCallFunc: () => {} // 空函数，避免报错
                },
                google_scholar: {
    ...
    class AutoPager {
        ...
        async insertPageContent(pageElements, tempDoc) {
            ...
            if (!insertNode && this.config.siteName === 'google') {
                // 如果仍然找不到，尝试 #center_col 作为最后后备
                insertNode = MyApi.getAllElements('css;#center_col')[0];
                if (!insertNode) insertNode = MyApi.getAllElements('css;footer')[0]; // 再尝试 footer
            }
            if (!insertNode) {
    ...
    const initGooglePage = () => {
        ...
        const createGoogleSearchBar = () => {
            ...
            if (navBar) {
                // 确保父容器是 flex 布局，以便 margin-left: auto 生效
