...
// @note         2026.2.21 V1.9.9 优化刷新延迟至50ms，最小间隔1000ms，新增搜索框回车刷新，新增搜索按钮和推荐词点击刷新
// ==/UserScript==
...
(function() {
    ...
    if (/baidu\.com/.test(hostname)) {
        ...
        // 监听搜索按钮点击（百度一下）—— 已包含在下方的通用点击监听中，但保留单独监听确保覆盖
        ...
        // 新增：监听翻页、加载更多按钮、搜索按钮、推荐关键词的点击，触发强制刷新
        ...
        document.addEventListener('keydown', function(e) {
            if (e.key === 'Enter' && !e.isComposing) {
                const target = e.target;
                if (target.matches && target.matches('#chat-textarea')) {
                    scheduleRefresh();
                }
            }
        }, true);
