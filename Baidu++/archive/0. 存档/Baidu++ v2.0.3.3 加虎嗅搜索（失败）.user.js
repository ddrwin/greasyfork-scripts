...
(function() {
    ...
    const initHuxiuPage = () => {
        ...
        // 清除 hash，避免重复触发
        window.location.hash = '';
        console.log('虎嗅网自动搜索：关键词 =', keyword);
        ...
        // 模拟点击事件（修复：使用 unsafeWindow 作为 view）
        const simulateClick = (element) => {
            try {
                // unsafeWindow 是原始页面 window，更可靠
                ...
                console.warn('MouseEvent 构造失败，使用 element.click() 回退', e);
                element.click(); // 降级方案，可能有效
            }
        };
        ...
        (async () => {
            try {
                // 检查搜索框是否已展开（输入框存在且可见）
                let searchInput = document.querySelector('.search-input input[placeholder="搜索文章"]');
                if (!searchInput || !searchInput.offsetParent) {
                    // 点击搜索按钮展开搜索框
                    const searchBtn = await waitForElement('.search-btn, .search-icon-container .svg-icon');
                    simulateClick(searchBtn);
                    // 等待搜索输入框出现
                    searchInput = await waitForElement('.search-input input[placeholder="搜索文章"]');
                }
                // 填入关键词
                ...
                simulateClick(searchSubmit);
            } catch (e) {
                console.error('虎嗅自动搜索失败:', e);
            }
        })();
    };
