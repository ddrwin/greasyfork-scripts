...
(function() {
    ...
    // ===================== 9. 虎嗅网增强模块（严格匹配，重试5次） =====================
    /**
     * 虎嗅网自动操作：当 URL 带有 #q=关键词 时，优先点击完全匹配的历史关键词；
     * 若不匹配或无历史，则自动填入关键词并搜索。
     * 重试次数增加至5次，确保面板可靠打开。
     */
    const initHuxiuPage = () => {
        // ---------- 辅助函数定义 ----------
        ...
        /**
         * 模拟完整鼠标点击（带坐标，触发 mousedown → mouseup → click）
        ...
        /**
         * 带重试的点击搜索按钮（重试5次，每次间隔500ms）
         * @param {HTMLElement} btn 搜索按钮元素
         * @param {number} maxRetry 最大重试次数（默认5）
