// ==UserScript==
// @name         为百度搜索结果页添加磁力、种子、网盘、软件、头条、哔哩哔哩、知乎、CSDN、Google 搜索按钮，为 Google 添加百度搜索按钮
// @description  为百度搜索结果页添加软件、网盘、财经、社交问答、Google、Bing 搜索按钮，一键跳转到多平台检索；在 Google 搜索结果页添加内联百度 / Bing 搜索框，在 Bing 搜索结果页添加内联百度 / Google 搜索框；支持去除百度广告和右侧栏，集成快科技搜索页中转修复驱动之家搜索问题；新增卡片美化样式、精准间距控制、强制刷新机制（点击翻页 / 更多 / 搜索触发），彻底解决筛选后按钮消失、间距无效问题，全功能兼容异步刷新场景
...
(function() {
    ...
    const MyApi = (() => {
        ...
        const getAllElementsByXpath = (xpath, contextNode = document, doc = document) => {
            ...
            try {
                ...
                console.error('[Pager] XPath解析失败:', xpath, err);
            }
            return result;
        };
    ...
    class AutoPager {
        ...
        async loadNextPage() {
            ...
            try {
                if (!this.nextPageUrl) {
                    console.log('[Pager] 无更多页面（未找到下一页链接）');
                    this.isLoading = false;
                    return;
                }
                ...
                console.log(`[Pager] 加载第${this.pageNum}页:`, nextUrl);
                const [err, respText] = await MyApi.http.get(nextUrl);
                if (err || !respText) {
                    console.error('[Pager] 页面请求失败:', err);
                    this.isLoading = false;
                    return;
                }
                ...
                console.log(`[Pager] 第${this.pageNum}页加载完成`);
            } catch (e) {
                console.error('[Pager] 翻页异常:', e);
            } finally {
                this.isLoading = false;
            }
        }
