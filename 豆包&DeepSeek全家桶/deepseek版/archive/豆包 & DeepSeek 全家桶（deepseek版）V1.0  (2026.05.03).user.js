...
(function() {
     ...
     *                         模块 1  统一偏好管理 (扩展)
     * ══════════════════════════════════════════════════════════════════════ */
    const PREF_KEY = 'ai_assistant_prefs_v2'; // 沿用主程序存储键
    const defaultPrefs = Object.freeze({
        ...
        // Usage 监控 (V8.6 迁移) — master_switch 未拆分，保持原语义
    ...
    function loadPrefs() {
        ...
        // 从旧 Token 脚本裸键迁移 (master_switch / display_mode)
        ...
        // 确保合并默认值
        const merged = Object.assign({}, defaultPrefs, saved);
        return merged;
    }
    ...
    /* ════════════════════════════════════════════════════════════════════════
     *                         模块 3  早期注入：matchMedia 劫持 (侧边栏防线一)
     *         必须在路由前、document-start 阶段执行，无需等待 DOM
     * ══════════════════════════════════════════════════════════════════════ */
    if (location.hostname === 'chat.deepseek.com') {
        ...
        LOG.boot('matchMedia 劫持已安装 (侧边栏守护)');
    }
    ...
    function bootUsageMonitor() {
        // ========== 偏好相关 ==========
        const currentPrefs = loadPrefs(); // 独立加载
        ...
        let currentMode = displayMode; // 本地缓存的模式
        ...
        function enableFloatWindow() {
            ...
            setDisplayMode(currentMode); // 确保模式同步
        }
        ...
        function registerUsageMenus() {
            ...
            usageMenuIds.push(GM_registerMenuCommand(`📊 完整功能: ${currentMaster ? '关闭' : '开启'}`, () => {
                ...
                registerUsageMenus(); // 重新注册以更新菜单文本
            }));
