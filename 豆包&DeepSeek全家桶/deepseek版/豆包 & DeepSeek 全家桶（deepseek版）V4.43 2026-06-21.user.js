// ==UserScript==
// @name          DeepSeek++  (DeepSeek & 豆包全家桶)
// @namespace    https://greasyfork.org/zh-CN/scripts/538175
// @version      4.3
// @description  侧边栏永恒守护；Usage 页千分位格式化 + 缓存命中率悬浮窗，统一偏好管理、站点控件增强、遥测拦截；
...
// @note         V4.1 修复 matchMedia 劫持时序问题 (移至文件最顶部执行，hostname 判断内置)
// @note         V4.2 劫持策略升级：所有 max-width 查询一律返回 false (覆盖 1200/1024/768 等全部断点)
// @note         V4.3 修复 GM grant 导致的沙箱隔离：劫持作用于 unsafeWindow，解决 matchMedia 失效问题
// @note         V4.41 豆包模型选项更新为"快速/专家/办公任务"；修复 /chat/ 路径识别；操作锁防死循环；菜单项搜索增加备用选择器
// @note         V4.42 菜单项点击优先原生 click（避免 simulateClick 触发 Violentmonkey content.js 报错导致点击失效）
// @note         V4.43 DeepSeek 新增"识图模式"(vision)：findElement/getCurrentValue/verify/GM菜单全更新，选项: 快速模型/专家模型/识图模型
// ==/UserScript==
...
(function() {
    ...
    let gLastActionTime = 0;      // V1.41: 模块级，供 action 箭头函数使用
    const ACTION_COOLDOWN = 800;  // V1.41: 冷却期(ms)，换会话后更快响应
    ...
    const defaultPrefs = Object.freeze({
        doubaoModel: '快速',       // V1.41: 豆包新版选项"快速/专家/办公任务"
    ...
    function loadPrefs() {
        ...
        // V1.41: 迁移旧版"思考"→新版"快速"
        if (saved.doubaoModel === '思考') {
            saved.doubaoModel = '快速';
        }
    ...
    const siteConfigs = {
        'doubao.com': {
            ...
            conversationPathTest: (p) => p.startsWith('/chat/') && !/^\/chat\/?$/.test(p),   // V1.41: 去掉了 p.length>6 限制
            controls: [{
                id: 'doubaoModel',
                ...
                findElement: () => {
                    // V1.41: 优先 data-valid-btn 精确定位（豆包新版 Radix UI）
                    const container = document.querySelector('[data-valid-btn="mode-select-action-btn"]');
                    if (container) {
                        const trigger = container.closest('button[aria-haspopup="menu"]');
                        if (trigger) return trigger;
                    }
                    const btns = document.querySelectorAll('button[aria-haspopup="menu"]');
                    return Array.from(btns).find(b => ['快速','专家','办公'].some(m => b.textContent.includes(m)));
                },
                getCurrentValue: btn => {
                    if (!btn) return null;
                ...
                action: async (pref, btn) => {
                    ...
                    for (let i = 0; i < 15; i++) {
                        ...
                        for (const sel of selectors) {
                            ...
                            target = Array.from(items).find(it => it.textContent.includes(pref));
                            if (target) break;
                        }
                        if (target) break;
                        // Portal 菜单可能在 body 顶层
                        const allElements = document.body.querySelectorAll('*');
                        target = Array.from(allElements).find(el =>
                            ['menuitem','option','radio'].includes(el.getAttribute('role') || '') &&
                            el.textContent.includes(pref) &&
                    ...
                    if (target) {
                        LOG.scan('豆包模型', `找到菜单项: ${target.textContent.trim()}`);
                        try { target.click(); } catch(e) { simulateClick(target); }
                    } else {
                        const allBtns = document.querySelectorAll('button');
                        const alt = Array.from(allBtns).find(b =>
                            b.textContent.trim() === pref && b.offsetParent !== null
                        );
                        if (alt) { LOG.scan('豆包模型', `兜底按钮: ${alt.textContent.trim()}`); simulateClick(alt); }
                        else { LOG.warn('豆包模型', `找不到"${pref}"，回退 trigger`); simulateClick(btn); }
                    }
                    gLastActionTime = Date.now();
                },
                verify: (target) => {
                    // V1.41: 优先 data-valid-btn 定位
                    let btn = null;
                    const container = document.querySelector('[data-valid-btn="mode-select-action-btn"]');
                    if (container) btn = container.closest('button[aria-haspopup="menu"]');
                    if (!btn) {
                        const btns = document.querySelectorAll('button[aria-haspopup="menu"]');
                        btn = Array.from(btns).find(b => ['快速','专家','办公'].some(m => b.textContent.includes(m)));
                    }
                ...
                manualTrigger: (e) => {
                    ...
                    if (txt.includes('办公')) return '办公任务';
                    return null;
                }
            }]
        },
            ...
            controls: [
                {
                    ...
                    steps: 5,
                    appliesTo: ['home'],
                    findElement: () => document.querySelector('div[data-model-type="expert"], div[data-model-type="default"], div[data-model-type="vision"]'),
                    getCurrentValue: () => {
                        ...
                        const vision = document.querySelector('div[data-model-type="vision"]');
                        if (vision && vision.getAttribute('aria-checked') === 'true') return 'vision';
                        return null;
                    },
                    isMatch: () => false,
                    action: async (pref) => {
                    ...
                    verify: (target) => {
                        ...
                        const expectedIndex = target === 'default' ? '0' : (target === 'expert' ? '1' : '2');
    ...
    function createSiteController(hostname, config) {
        ...
        let scanTimer = null;
        let isApplying = false;        // V1.41: 操作锁
        ...
        function refreshMenus() {
            ...
            if (hostname.includes('doubao.com')) {
                // V1.41: 豆包新版选项"快速/专家/办公任务"
                menuIds.push(GM_registerMenuCommand(`⚡ 快速${p.doubaoModel === '快速' ? ' [当前]' : ''}`, () => {
                    updatePreference('doubaoModel', '快速');
                    scanAll();
                }));
                menuIds.push(GM_registerMenuCommand(`🚀 专家${p.doubaoModel === '专家' ? ' [当前]' : ''}`, () => {
                    updatePreference('doubaoModel', '专家');
                    scanAll();
                }));
                menuIds.push(GM_registerMenuCommand(`📋 办公任务${p.doubaoModel === '办公任务' ? ' [当前]' : ''}`, () => {
                    updatePreference('doubaoModel', '办公任务');
                    scanAll();
                }));
                ...
                menuIds.push(GM_registerMenuCommand(`👁 识图模型${p.deepseekModel === 'vision' ? ' [当前]' : ''}`, () => {
                    updatePreference('deepseekModel', 'vision');
                    scanAll();
                }));
        ...
        const Scheduler = {
            ...
            async run(pageType, reason) {
                if (isApplying) { LOG.scheduler(`⏭️ 忽略工作流[${pageType}]：操作锁生效中`); return; }
                isApplying = true;
                try {
        ...
        function scheduleScan(reason) {
            // V1.41: 操作锁 + 冷却期
            if (isApplying) { LOG.scheduler(`⏭️ 忽略扫描[${reason}]：操作锁生效中`); return; }
            const now = Date.now();
            if (now - gLastActionTime < ACTION_COOLDOWN) { return; }
