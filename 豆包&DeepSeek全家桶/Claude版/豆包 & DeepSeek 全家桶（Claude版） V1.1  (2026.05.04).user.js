...
// @license      MIT
// @note         2026.05.03 V1.0 合并自 V4.0(自动专家主程序) + v2.5(侧边栏永恒守护) + V8.6(Usage千分位&悬浮窗)。行为等价合并，未做项目卡§5的偏好解耦优化（留待专项处理）。
// @note         V1.1 修复 @grant 沙箱隔离导致 matchMedia 劫持失效：添加 @grant unsafeWindow，劫持目标改为 pageWindow（真实 window），同时劫持脚本 window 以防万一。执行时序从路由器分支内移到模块 5 顶部。来源：deepseek 版 V1.0→V1.3 事故复盘。
// ==/UserScript==
...
(function() {
    ...
    // V1.1 修复：劫持目标从 window 改为 pageWindow（页面真实 window），
    // 同时劫持脚本 window 以防万一。来源：deepseek 版 V1.0→V1.3 事故复盘
    function installMatchMediaInterceptor() {
        ...
        LOG.boot('【侧边栏守护】防线一已安装（matchMedia 劫持，双作用域）');
    }

    // V1.1 修复：防线一从路由器分支内移到此处立即执行，确保在 React 初始化前生效
    // （原 V1.0 调用点在模块 7 路由器分支内，被前序模块的 GM_getValue IPC 延迟）
    ...
    try {
        ...
        // ----- 路由分支 2：DeepSeek 聊天 -----
            ...
            // V1.1 注意：matchMedia 劫持已在模块 5 顶部提前执行，此处不再重复调用
            // （原调用点在此，V1.1 移至模块 5 顶部以确保 React 初始化前生效）
